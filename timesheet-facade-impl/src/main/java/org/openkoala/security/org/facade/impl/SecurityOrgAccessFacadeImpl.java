...
public class SecurityOrgAccessFacadeImpl implements SecurityOrgAccessFacade {
    ...
    @SuppressWarnings("unchecked")
	@Override
    public Page<EmployeeUserDTO> getEmployeeUser(int page, int pagesize) {
        StringBuilder jpql = new StringBuilder("SELECT new org.openkoala.security.org.facade.dto.EmployeeUserDTO(_user.id,_user.userAccount,_employee.name) FROM EmployeeUser _user left join _user.employee as _employee");

        Page<EmployeeUserDTO> results = getQueryChannelService().createJpqlQuery(jpql.toString())//
