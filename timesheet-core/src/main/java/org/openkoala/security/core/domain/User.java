...
public class User extends Actor {
    ...
    public static Long getEmployeeId(Long UserId) {
        String sql = "SELECT employee_id FROM ks_actors WHERE id=?";
        List<BigInteger> list = getRepository().createSqlQuery(sql).setParameters(UserId).list();
        Long employeeId = 0l;
        if(list.size() > 0) {
            employeeId = Long.valueOf(list.get(0).toString());
        }
        return employeeId;
    }
}