...
public abstract class Party extends OrganizationAbstractEntity {
	...
	public static <P extends Party> boolean isExistName(Class<P> clazz, String name, Date date) {
		List<P> parties = getRepository().createCriteriaQuery(clazz).eq("name", name).le("createDate", date).gt("terminateDate", date).list();
		return !parties.isEmpty();
	}
	...
	public static Party getByName(String name) {
		List<Party> partyList = getRepository().findByProperty(Party.class, "name", name);
