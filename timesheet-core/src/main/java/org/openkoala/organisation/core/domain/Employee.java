...
public class Employee extends Party {
	...
	public static <P extends Party> boolean isExistName(Class<P> clazz, String name,String email) {
		List<P> parties = getRepository().createCriteriaQuery(clazz).eq("person.email",email).eq("name", name).list();
		return !parties.isEmpty();
	}
	public static <P extends Party> Party getByNameAndEmail(Class<P> clazz, String name,String email) {
		List<P> parties = getRepository().createCriteriaQuery(clazz).eq("person.email",email).eq("name", name).list();
		return parties.get(0);
	}

	public static List<Long> getUserIdList(List<Employee> employees) {
		String ids = "";
		...
		String sql = "SELECT id FROM ks_actors WHERE employee_id in ("+ids+")";
		List<BigInteger> list = getRepository().createSqlQuery(sql).list();
		List<Long> userIdList = new ArrayList<>();
		for(BigInteger id: list) {
			userIdList.add(Long.valueOf(id.toString()));
		}
		return userIdList;
	}
}
