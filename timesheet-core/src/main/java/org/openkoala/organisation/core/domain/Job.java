...
public class Job extends Party {
	...
	public static <P extends Party> boolean isExistName(Class<P> clazz, String name, Date date) {
		List<P> parties = getRepository().createCriteriaQuery(clazz).eq("category","JOB").eq("name", name).le("createDate", date).gt("terminateDate", date).list();
		return !parties.isEmpty();
	}

	public static  <P extends Party> Job getByName(Class<P> clazz, String name) {
		return (Job) getRepository().createCriteriaQuery(clazz).eq("category","JOB").eq("name", name).list().get(0);
	}
}
