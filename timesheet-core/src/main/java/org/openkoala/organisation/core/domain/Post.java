...
public class Post extends Party {
	...
	private boolean nameExist() {
		Date now = new Date();
		CriteriaQuery query = getRepository().createCriteriaQuery(Post.class).eq("organization.id", getOrganization().getId()).eq("name", getName()).le("createDate", now).gt("terminateDate", now);
	...
	public static <P extends Party> boolean isExistName(Class<P> clazz, String name,Long deptId,  Date date) {
		List<P> parties = getRepository().createCriteriaQuery(clazz).eq("organization.id", deptId).eq("name", name).le("createDate", date).gt("terminateDate", date).list();
		return !parties.isEmpty();
	}

	public static <P extends Party> Party getByName(Class<P> clazz, String name) {
		List<P> parties = getRepository().createCriteriaQuery(clazz).eq("category", "POST").eq("name", name).list();
